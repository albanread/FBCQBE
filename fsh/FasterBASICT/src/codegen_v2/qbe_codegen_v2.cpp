...
void QBECodeGeneratorV2::emitGlobalVariable(const VariableSymbol* varSymbol) {
    ...
    // Get QBE type
    std::string qbeType = typeManager_->getQBEType(varSymbol->typeDesc.baseType);
    
    // Get default value
    std::string defaultValue = typeManager_->getDefaultValue(varSymbol->typeDesc.baseType);
